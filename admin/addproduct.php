...
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
    $id = $_GET['id'];
    $getsql = mysqli_query($con, "SELECT * FROM lab_tests WHERE `s.no`='$id'");
    if ($getsql && mysqli_num_rows($getsql) > 0) {
        $testData = mysqli_fetch_assoc($getsql);
    }
}
...
if (isset($_POST['submit'])) {
    ...
    if (empty($test_name)) {
        $message = '<div class="alert alert-danger">Test name is required!</div>';
    } else {
        if (!empty($id)) {
            $sql = "UPDATE lab_tests SET test_name='$test_name' WHERE `s.no`='$id'";
        } else {
            $sql = "INSERT INTO lab_tests (test_name) VALUES ('$test_name')";
        }

        if (mysqli_query($con, $sql)) {
            $message = '<div class="alert alert-success">Lab test saved successfully!</div>';
            $testData['test_name'] = '';
            ...
            <div class="offcanvas-body mx-0 flex-grow-0">
              <div class="container" style="border: 1px solid lightgray; padding: 37px; border-radius: 5px">
                <form method="post" id="labTestForm">
                  <center><h4><?= $id ? 'Edit' : 'Add' ?> Lab Test</h4></center>
                  ...
                  <div class="row">
                    <div class="col-md-6">
                      <div class="mb-3">
                        ...
                        <input type="text" autocomplete="off" class="form-control" placeholder="Enter Test Name" name="test_name" id="test_name" value="<?= htmlspecialchars($testData['test_name']); ?>" required>
  ...
  <script>
    $(document).ready(function () {
      $('#labTestForm').on('submit', function (e) {
        ...
        if (name === '') {
          e.preventDefault();
          $('#errorMsg').text('Test name cannot be blank or whitespace.');
        } else {
          $('#errorMsg').text('');
        }
      });
